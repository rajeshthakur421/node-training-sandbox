...
module.exports={
    ...
    addProducts:(req,res)=>{
        ...
        connection.query(`INSERT INTO products(id, products_name, product_qty, products_description) VALUES (0,'${products_name}','${product_qty}','${products_description}')`,(err,result)=>{
            if(err){
                res.send({error:true,message:err.message});
            }else{
                if(result.affectedRows>0){
                    res.send({error:false,message:"New products created with Id "})
                }else{
    ...
    updateProducts:(req,res)=>{
        ...
        connection.query(`UPDATE products SET products_name='${products_name}' WHERE id=${id}`,(err,result)=>{
            if(err){
                res.send({error:true,message:err.message});
            }else{
                if(result.affectedRows>0){
